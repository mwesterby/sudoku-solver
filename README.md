...
### Running the Tests
The tests are located in [`/src/test/java/solver/`](./src/test/java/solver). Use maven to run the tests with:
```
mvn test
```
...
The `solve()` method will print the solved board to the console (along with a running log of each value and the coordinate it is added to). e.g. for the unsolved board above, you may see something like this:
...
There is currently no validation of the unsolved board which is passed in. This means the program may get stuck if it's trying to solve an invalid board.
